...
CREATE TABLE IF NOT EXISTS `messages` (
    ...
    `sender_uid` BIGINT NOT NULL COMMENT '发送者ID',
    `sender_name` VARCHAR(100) COMMENT '发送者名称',
    `sender_face` VARCHAR(500) COMMENT '发送者头像',
    `conversation_uid` BIGINT COMMENT '会话对端UID',
    `conversation_name` VARCHAR(100) COMMENT '会话对端名称',
    `conversation_face` VARCHAR(500) COMMENT '会话对端头像',
    `is_from_self` TINYINT(1) DEFAULT 0 COMMENT '是否自己发出',
    `content` TEXT NOT NULL COMMENT '消息内容',
    `reply_status` TINYINT DEFAULT 0 COMMENT '回复状态',
    `reply_content` TEXT COMMENT '回复内容',
    `is_ai_reply` TINYINT(1) DEFAULT 0 COMMENT '是否AI回复',
    `msg_type` INT DEFAULT 1 COMMENT '消息类型',
    ...
    KEY `idx_conversation_uid` (`conversation_uid`),
    KEY `idx_is_from_self` (`is_from_self`),
    KEY `idx_reply_status` (`reply_status`),
    KEY `idx_deleted_at` (`deleted_at`)
